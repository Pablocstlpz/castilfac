...
export const getPedidosByOperario = async (req, res) => {
    try {
        ...
        console.log(error);
        //devuelvo un mensaje de error
        res.status(500).json({ message: 'Error al obtener los pedidos asignados a este operario' });
    }
}
...
export const getPedidosByEmpresa = async (req, res) => {
    try {
        ...
        //valido que haya pedidos, si no hay digo que no hay
        if (pedidos.length === 0) {
            return res.status(404).json({ message: 'No hay pedidos de esta empresa' });
        }
        ...
        console.log(error);
        //devuelvo un mensaje de error
        res.status(500).json({ message: 'Error al obtener los pedidos de esta empresa' });
    }
}
