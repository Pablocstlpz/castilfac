...
export const createEmpresa = async (req, res) => {
    try {
        ...
        if (!codigo_postal.match(/^[0-9]{5}$/)) {
            return res.status(400).json({ message: 'El codigo postal debe ser un codigo postal valido' });
        }

        //valido que la ciudad sea una ciudad valida
        if (!ciudad.match(/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/)) {
            return res.status(400).json({ message: 'La ciudad debe ser una ciudad valida' });
        }

        //valido que la provincia sea una provincia valida
        if (!provincia.match(/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/)) {
            return res.status(400).json({ message: 'La provincia debe ser una provincia valida' });
        }
...
export const updateEmpresa = async (req, res) => {
    try {
        ...
        //recojo los datos del body
        const { nombre_comercial, razon_social, nif, email, telefono, direccion, codigo_postal, ciudad, provincia, suscripcion_activa, fecha_vencimiento, activo, logo_url } = req.body;
...
export const deleteEmpresaCorreo = async (req, res) => {
    try {
        ...
        console.log(error);
        //devuelvo un mensaje de error
        res.status(500).json({ message: 'Error al eliminar la empresa por correo' });
    }
}